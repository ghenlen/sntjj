...
<head>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			var tpl2= $("#treeTableTpl2").html().replace(/(\/\/\<!\-\-)|(\/\/\-\->)/g,"");
			var data = ${fns:toJson(list)}, rootId = "${not empty office.id ? office.id : '0'}";
			addRow("#treeTableList", tpl,tpl2, data, rootId, true);
			$("#treeTable").treeTable({expandLevel : 5});
		});
		function addRow(list, tpl,tpl2, data, pid, root){
			for (var i=0; i<data.length; i++){
				var row = data[i];
				if ((${fns:jsGetVal('row.parentId')}) == pid){
					if(row.id=='ea0a68f17c54494582324628c593ded4'){
						$(list).append(Mustache.render(tpl2, {
							dict: {
					...
					addRow(list, tpl,tpl2, data, row.id);
				}
			}
		}
	</script>
...
<body>
	...
	<script type="text/template" id="treeTableTpl2">
		<tr id="{{row.id}}" pId="{{pid}}">
			<td><a href="${ctx}/sys/office/form?id={{row.id}}">{{row.name}}</a></td>
			<td>{{row.area.name}}</td>
			<td>{{row.code}}</td>
